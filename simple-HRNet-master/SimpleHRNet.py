...
class SimpleHRNet:
    ...
    def predict(self, image, x1 = 0.0, x2 = 0.0, y1 = 0.0, y2 = 0.0):
        """
        ...
        if len(image.shape) == 3:
            return self._predict_single(image, x1, x2, y1, y2)
        elif len(image.shape) == 4:
            return self._predict_batch(image)
        else:
            raise ValueError('Wrong image format.')

    def _predict_single(self, image, x1 = 0.0, x2 = 0.0, y1 = 0.0, y2 = 0.0):
        if not self.multiperson:
            # old_res = image.shape
            ...
            heatmaps = np.zeros((nof_people, self.nof_joints, self.resolution[0] // 4, self.resolution[1] // 4), dtype=np.float32)
            ...
            images[0] = self.transform(image_crop)
            # images[0] = torch.swapaxes(torch.swapaxes(torch.from_numpy(cv2.resize(np.swapaxes(np.swapaxes(self.transform(image_crop).numpy(), 0, 1), 1, 2), \
            #                                          (self.resolution[1], self.resolution[0]), interpolation=self.interpolation)), 0, 2), 1, 2)
            boxes[0] = [x1_new, y1_new, x2_new, y2_new]
                # [int((x1_new + x2_new) / 2 - self.resolution[1] / 2), \
                #         int((y1_new + y2_new) / 2 - self.resolution[0] / 2), \
                #         int((x1_new + x2_new) / 2 - self.resolution[1] / 2) + self.resolution[1], \
                #         int((y1_new + y2_new) / 2 - self.resolution[0] / 2) + self.resolution[0]]
