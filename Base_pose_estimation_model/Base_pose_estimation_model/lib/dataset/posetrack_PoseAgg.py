...
def video2filenames_new(annot_dir):
    output = {}
    L = {}
    for json_name in os.listdir(annot_dir):
        print(json_name)
        output['images/' + annot_dir.split('/')[-2] + '/' + json_name.split('.json')[0]] = json_name
        L['images/' + annot_dir.split('/')[-2] + '/' + json_name.split('.json')[0]] = len(json.load(open(annot_dir + json_name, 'r'))['images'])
    return output, L
...
class PoseTrackDataset(JointsDataset):
    ...
    def evaluate(self, cfg, preds, output_dir, boxes, img_path, *args, **kwargs):
        ...
        for key in img_path:
            ...
            frame_num = int(prev_nm.replace('.png','')) # int(prev_nm.replace('.jpg','')) # Allen
        ...
        out_data = {}
        out_filenames, L = video2filenames_new(annot_dir) # Allen
