...
def locate_joints_keypoints_allhumans_former_img(former_img_example_bbox_list, former_img, Num_samples_former_img, former_img_example_skeleton_list, curr_img_joints, curr_img_keypoints, curr_img_keypoints_mask, max_motion_vector, joint_color, keypoint_color, draw_rectangle_or_not=1):
    ...
    for human_idx in range(len(former_img_example_bbox_list)):
        ...
        if draw_rectangle_or_not:
            vis_img_joints = cv2.rectangle(vis_img_joints, (int(former_img_example_bbox_list[human_idx]['bbox'][0])*3, int(former_img_example_bbox_list[human_idx]['bbox'][1])*3),
                                           (int(former_img_example_bbox_list[human_idx]['bbox'][0] + former_img_example_bbox_list[human_idx]['bbox'][2])*3, int(former_img_example_bbox_list[human_idx]['bbox'][1] + former_img_example_bbox_list[human_idx]['bbox'][3])*3), joint_color[human_idx], 3)
        if draw_rectangle_or_not:
            vis_img_keypoints = cv2.rectangle(vis_img_keypoints, (int(former_img_example_bbox_list[human_idx]['bbox'][0])*3, int(former_img_example_bbox_list[human_idx]['bbox'][1])*3),
                                              (int(former_img_example_bbox_list[human_idx]['bbox'][0] + former_img_example_bbox_list[human_idx]['bbox'][2])*3, int(former_img_example_bbox_list[human_idx]['bbox'][1] + former_img_example_bbox_list[human_idx]['bbox'][3])*3), joint_color[human_idx], 3)
...
def convert_from_17_to_15(curr_human_anno):
    ...
    for curr_human_anno_item in curr_human_anno:
        if curr_human_anno.tolist().index(curr_human_anno_item.tolist()) in manually_labelled_semantics_16_points.keys():
            curr_human_anno_converted[manually_labelled_semantics[manually_labelled_semantics_16_points[curr_human_anno.tolist().index(curr_human_anno_item.tolist())]]] = curr_human_anno_item[:2]
    if (3 in curr_human_anno_converted) and (4 in curr_human_anno_converted):
        left_shoulder_coord = curr_human_anno_converted[3]
        right_shoulder_coord = curr_human_anno_converted[4]
        neck_coord = (left_shoulder_coord + right_shoulder_coord) / 2
        curr_human_anno_converted[2] = neck_coord
    if (1 in curr_human_anno_converted) and (2 in curr_human_anno_converted):
        curr_human_anno_converted[0] = [2 * curr_human_anno_converted[1][0] - curr_human_anno_converted[2][0], curr_human_anno_converted[2][1]] # 2 * curr_human_anno_converted[1] - curr_human_anno_converted[2]
    for curr_human_anno_converted_key in curr_human_anno_converted:
        curr_human_anno_converted[curr_human_anno_converted_key] = curr_human_anno_converted[curr_human_anno_converted_key][-1::-1]
...
def convert_from_17_to_15_joints_and_heatmaps(curr_human_anno, heatmaps, isotropicGrayscaleImage, img_width, img_height):
    ...
    heatmaps_result = np.zeros((heatmaps.shape[0], len(manually_labelled_semantics), heatmaps.shape[2], heatmaps.shape[3]))
    for curr_human_anno_item in curr_human_anno:
        if curr_human_anno.tolist().index(curr_human_anno_item.tolist()) in manually_labelled_semantics_16_points.keys():
            curr_human_anno_converted[manually_labelled_semantics[manually_labelled_semantics_16_points[curr_human_anno.tolist().index(curr_human_anno_item.tolist())]]] = curr_human_anno_item#[:2]
            heatmaps_result[0, manually_labelled_semantics[manually_labelled_semantics_16_points[curr_human_anno.tolist().index(curr_human_anno_item.tolist())]], :, :] = heatmaps[0, curr_human_anno.tolist().index(curr_human_anno_item.tolist()), :, :]
...
def mask_out_low_vis_regions_and_pose_estimation(latter_img_joint_level_vis_scores, latter_img_joints, latter_img_skeletons, latter_img_bboxes, latter_img, model):
    ...
    for latter_img_bbox_idx in range(len(latter_img_bboxes)):
        ...
        for latter_img_neighbor_bbox_idx in [x for x in np.where(latter_img_inter_bbox_iou[latter_img_bbox_idx, :] > 0.4)[0].tolist() if x != latter_img_bbox_idx]:
            ...
            for neighbor_joint_idx in range(len(latter_img_joints[latter_img_neighbor_bbox_idx])):

                neighbor_joint_idx_hori, neighbor_joint_idx_vert = latter_img_joints[latter_img_neighbor_bbox_idx][neighbor_joint_idx][0], latter_img_joints[latter_img_neighbor_bbox_idx][neighbor_joint_idx][1]
            ...
            for curr_joint_idx in range(len(latter_img_joints[latter_img_bbox_idx])):
                native_joints.append(latter_img_joints[latter_img_bbox_idx][curr_joint_idx][:2] + [latter_img_joint_level_vis_scores[latter_img_bbox_idx][curr_joint_idx]])
                curr_joint_idx_hori, curr_joint_idx_vert = latter_img_joints[latter_img_bbox_idx][curr_joint_idx][0], latter_img_joints[latter_img_bbox_idx][curr_joint_idx][1]
                if curr_joint_idx_hori >= neighbor_bbox_left and curr_joint_idx_hori < neighbor_bbox_left + neighbor_bbox_width and \
                    ...
                    native_joints_inside_common_area.append(latter_img_joints[latter_img_bbox_idx][curr_joint_idx][:2] + [latter_img_joint_level_vis_scores[latter_img_bbox_idx][curr_joint_idx]])
                else:
                    native_joints_outside_common_area.append(latter_img_joints[latter_img_bbox_idx][curr_joint_idx][:2] + [latter_img_joint_level_vis_scores[latter_img_bbox_idx][curr_joint_idx]])
            if len(curr_alien_set) < 3:
                continue
            while np.mean([x[2] for x in curr_alien_set]) < np.mean([x[2] for x in native_joints_inside_common_area]):
        ...
        if np.sum(masked_joints[0][:, 2]) > np.sum(unmasked_joints[0][:, 2]):
            ...
            latter_img_curr_bbox_15_keypoints = convert_from_17_to_15(masked_joints)
            for latter_img_joints_idx in range(len(latter_img_joints[latter_img_bbox_idx])):
                if latter_img_joints_idx in latter_img_curr_bbox_15_keypoints:
                    latter_img_joints[latter_img_bbox_idx][latter_img_joints_idx][:2] = latter_img_curr_bbox_15_keypoints[latter_img_joints_idx]
...
def bbox_propagation(curr_box_corner_points, curr_forward_flow):
    ...
    left_coord, right_coord = curr_box_corner_points[0], min([curr_box_corner_points[0] + curr_box_corner_points[2], curr_forward_flow.shape[1] - 2])
    top_coord, bottom_coord = curr_box_corner_points[1], min([curr_box_corner_points[1] + curr_box_corner_points[3], curr_forward_flow.shape[0] - 2])
