...
#########################################################################################################################
################################# import for self inference pose model ##################################################
#########################################################################################################################
from base_pose_model_demo import *

self_inference_pose_model_args = argparse.Namespace(cfg='/home/allenyljiang/Downloads/Base_pose_estimation_model/experiments/posetrack/hrnet/temporal_pose_aggregation.yaml', opts=['OUTPUT_DIR', '/home/allenyljiang/Downloads/Base_pose_estimation_model/val_results/out/', 'LOG_DIR', '/home/allenyljiang/Downloads/Base_pose_estimation_model/val_results/log/', 'DATASET.NUM_LABELED_VIDEOS', '-1', 'DATASET.NUM_LABELED_FRAMES_PER_VIDEO', '-1', 'DATASET.JSON_DIR', '/home/allenyljiang/Downloads/Base_pose_estimation_model/supp_files/posetrack18_json_files/', 'DATASET.IMG_DIR', '/home/allenyljiang/Downloads/Dataset/PoseTrack2018/', 'TEST.MODEL_FILE', '/home/allenyljiang/Downloads/Base_pose_estimation_model/val_results/out/posetrack/pose_hrnet/w48_384x288_adam_lr1e-4_PoseWarper_train/final_state.pth', 'TEST.COCO_BBOX_FILE', '/home/allenyljiang/Downloads/Base_pose_estimation_model/supp_files/posetrack18_precomputed_boxes/val_boxes.json', 'POSETRACK_ANNOT_DIR', '/home/allenyljiang/Downloads/Dataset/PoseTrack2018/posetrack_annotations/annotations/val/', 'TEST.USE_GT_BBOX', 'True', 'EXPERIMENT_NAME', '', 'DATASET.IS_POSETRACK18', 'True', 'TEST.IMAGE_THRE', '0.2', 'PRINT_FREQ', '5000'], modelDir='', logDir='', dataDir='', prevModelDir='')
...
#########################################################################################################################
###################################### load crowdpose estimator #########################################################
#########################################################################################################################
# from finetuned_pose_estimator.inferencer_demo import inferencer
...
def associate_ids_former_latter_img(former_img, former_img_bboxes, former_img_keypoints, former_img_keypoints_mask, latter_img, latter_img_bboxes, latter_img_keypoints, latter_img_keypoints_mask):
    ...
    for former_img_bbox in former_img_bboxes + latter_img_bboxes:
        former_img_bbox_enlarged = enlarge_bbox(copy.deepcopy(former_img_bbox), 0.25)
    ...
    need_upscale_here = 0
    if former_img_region.shape[0] >= 1000 or former_img_region.shape[1] >= 1000:
        former_img_region = cv2.resize(former_img_region, (0, 0), fx=0.75, fy=0.75)
        latter_img_region = cv2.resize(latter_img_region, (0, 0), fx=0.75, fy=0.75)
        need_upscale_here = 1
    ...
    with torch.no_grad():
        ...
        del former_img_region, latter_img_region #, flow_low_v1, flow_up_v1
        torch.cuda.empty_cache()

    if need_upscale_here == 1:
        flow_up = cv2.resize(flow_up, (former_img[former_img_all_bboxes_top:former_img_all_bboxes_bottom, former_img_all_bboxes_left:former_img_all_bboxes_right, :].shape[1],
                                       former_img[former_img_all_bboxes_top:former_img_all_bboxes_bottom, former_img_all_bboxes_left:former_img_all_bboxes_right, :].shape[0]))
    ...
    # propagate boxes in former image to latter image   lefttop, righttop, leftbottom, rightbottom
    ...
    for former_img_bbox_propagated_to_latter_img_idx in range(len(former_img_bboxes_propagated_to_latter_img)):
        for latter_img_bbox_idx in range(len(latter_img_bboxes)):
            ...
            latter_img_bbox = copy.deepcopy(latter_img_bboxes[latter_img_bbox_idx])
    ...
    # if former img describe completely different contexts with latter image
    if former_latter_img_bbox_cost_matrix_max_value <= 0:
        return "Former and latter image describe different scene!"
...
#########################################################################################################################
bbox_all_videos = 'demo_data/PoseTrack2018/box_predictions/val/val_boxes.json' # boxes in all images from all videos
curr_split = 'val'
src_video_dir = '/media/allenyljiang/Extreme_SSD/PoseTrack2018/images'
...
for curr_video_skeleton_pred_file in os.listdir(os.path.join(skeleton_pred_dir, curr_split)):
    print(curr_video_skeleton_pred_file)
    ...
    frame_indices = sorted(os.listdir(os.path.join(src_video_dir, curr_split, video_name)), key=lambda x:int(x.split('.png')[0]))
    for curr_img_pair_first_idx in range(0, len(frame_indices) - 1 - 2 * temporal_stride_between_imgs_in_each_pair):
        print('frame pair index: ' + str(curr_img_pair_first_idx))
        ###########################################################################################################################################################
        ################################ visualize joints and keypoints in former image ###########################################################################
        ###########################################################################################################################################################
        former_img_name = frame_indices[curr_img_pair_first_idx]
        latter_img_name = frame_indices[curr_img_pair_first_idx + temporal_stride_between_imgs_in_each_pair]
        third_img_name = frame_indices[curr_img_pair_first_idx + temporal_stride_between_imgs_in_each_pair * 2]
        ...
        # restrict the coords of boxes to lie in image
        former_img_bboxes = restrict_coords(former_img_bboxes, former_img)
        latter_img_bboxes = restrict_coords(latter_img_bboxes, latter_img)
        third_img_bboxes = restrict_coords(third_img_bboxes, third_img)
        ...
        if len([x for x in curr_video_baseposemodel_skeletons if (former_img_name in x['image']['name'])]) == 0 or \
                len([x for x in curr_video_baseposemodel_skeletons if (latter_img_name in x['image']['name'])]) == 0 or \
                len([x for x in curr_video_baseposemodel_skeletons if (third_img_name in x['image']['name'])]) == 0:
            continue
        ...
        former_img_skeletons = restrict_coords_skeleton(former_img_skeletons, former_img)
        latter_img_skeletons = restrict_coords_skeleton(latter_img_skeletons, latter_img)
        third_img_skeletons = restrict_coords_skeleton(third_img_skeletons, third_img)
        ...
        if latter_img_joint_level_vis_scores == "Former and latter image describe different scene!" or \
            third_img_joint_level_vis_scores == "Former and latter image describe different scene!":
            out_file = open(os.path.join(skeleton_pred_refined_dir, curr_split, curr_video_skeleton_pred_file), "w")
            json.dump({'annolist': curr_video_baseposemodel_skeletons}, out_file)
            out_file.close()
            continue
        ...
        ###########################################################################################################################################################
        latter_img_joints, latter_img_skeletons = mask_out_low_vis_regions_and_pose_estimation(latter_img_joint_level_vis_scores, latter_img_joints, latter_img_skeletons, latter_img_bboxes, latter_img, os.path.join(src_video_dir, curr_split, video_name, latter_img_name), self_inference_pose_model_model)
